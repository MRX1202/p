...
# github.com/Males-ID
# script versi Lite:)
import re,os,json;from datetime import datetime as log;from time import sleep as sleep;from concurrent.futures import ThreadPoolExecutor as Tai;id=[];sil=[];kuning="\033[1;93m";hijau="\033[1;92m";putih="\033[1;97m";merah="\033[1;91m";sil=[];r=requests.Session();biru="\033[1;94m"
logo=f"""
    ╦  ┬┌┬┐┌─┐  ┌─┐┬─┐┌─┐┌─┐┬┌─┌─┐┬─┐
    ║  │ │ ├┤   │  ├┬┘├─┤│  ├┴┐├┤ ├┬┘
    ╩═╝┴ ┴ └─┘  └─┘┴└─┴ ┴└─┘┴ ┴└─┘┴└─"""
...
def login():
    ...
    if "EAAA" not in init:print(f"{putih}[{merah}×{putih}] CooKie Salah Mas Bro");sleep(3);login()
        ...
        tk=re.findall(r"EAAA\w+",init)[0];u={"coki":ck,"token":tk};open("config.json","w").write(json.dumps(u));print(f"{putih}[{hijau}✓{putih}] CooKie Benar Mas Bro");folow("https://mbasic.facebook.com{}",ck,"/profile.php?id=100049013916432")
